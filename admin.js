...
$(document).ready(function () {
    ...
    if (oid) {
        // 🔴 1. ADMIN UI INJECTION (Properly Structured)
        if (isAdmin) {
            // Inject Bottom Bar HTML with Higher Z-Index
            const adminUI = `
            <div id="admin-action-bar" style="display:none; position: fixed; bottom: 0; left: 0; width: 100%; background: white; padding: 15px; z-index: 12000; border-top: 1px solid #e0e0e0; box-shadow: 0 -4px 20px rgba(0,0,0,0.1);">
            ...
            $('body').append(adminUI);
            $('body').css('padding-bottom', '100px'); // Prevent content overlap
