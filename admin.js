const scriptURL = "https://script.google.com/macros/s/AKfycbx0hYb7RFMs6K9ihsxUqgn-OwDCfNWI-GX9xs3NeTLXD9VDKbsO63PDP82MEpfLdvh9KQ/exec";
...
window.updateOrder = function (oid, status, trackingNum = null, skipConfirm = false, customDate = null) {
    if (!skipConfirm && !trackingNum && !customDate && !confirm(`Mark '${status}'?`)) return;

    let updates = JSON.parse(localStorage.getItem('pendingUpdates') || "[]");

    // മറ്റ് അപ്ഡേറ്റുകൾ (meta, paidNum) അല്ലാത്ത, നിലവിലുള്ള സ്റ്റാറ്റസ് മാറ്റം കണ്ടുപിടിക്കുന്നു
    let existingIndex = updates.findIndex(item => item.oid === oid && item.action !== 'meta' && item.action !== 'paidNum');
...
function injectLeftDrawer() {
    ...
    if ($('#btn-open-drawer').length === 0) {
        ...
        <button id="btn-open-drawer" onclick="toggleLeftDrawer()" 
            style="position: fixed; top: 12px; left: 15px; z-index: 1030; background: #fff; border: 2px solid #e2e8f0; border-radius: 8px; width: 40px; height: 40px; display: flex; align-items: center; justify-content: center; box-shadow: 0 2px 5px rgba(0,0,0,0.1); cursor: pointer; color: #0f172a;">
